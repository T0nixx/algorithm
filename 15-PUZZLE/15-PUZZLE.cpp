...
/*
int best;
void dfs(pair<vector<int>, int> here, const vector<int> &sorted, int steps) {
    if (steps >= best) return;

    if (here.first == sorted) {
        best = steps;
        return;
    }
    int change[4] = {-4, -1, 1, 4};
    for (int i = 0; i < 4; i++) {
        ...
        if (there_blank < 0 || there_blank > 15) continue;
        vector<int> there_puzzle = here.first;
        swap(there_puzzle[there_blank], there_puzzle[here.second]);
        pair<vector<int>, int> there = make_pair(there_puzzle, there_blank);
        dfs(there, sorted, steps + 1);
    }
}

int ids(const vector<int> &initial_puzzle, int initial_blank, int growth_step) {
    vector<int> sorted = initial_puzzle;
    sort(sorted.begin(), sorted.end());
    pair<vector<int>, int> start = make_pair(initial_puzzle, initial_blank);
    for (int limit = 4;; limit += growth_step) {
        best = limit + 1;
        dfs(start, sorted, 0);
...
int main() {
    ...
    // int output2 = ids(initial_puzzle, initial_blank, 3);
    // cout << output2 << endl;
